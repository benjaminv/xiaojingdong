...
if(in_array($action, $ui_arr))
{
	...
	else
	{
	 ...
	 $supplier_id_sql = "SELECT `supplier_id` FROM  " . $GLOBALS['ecs']->table('supplier') . " WHERE `user_id`='$user_id'";
		...
		if($_FILES['headimg']['size'] > 2048000)
		{
		 ...
		 show_message("头像文件过大", $_LANG['profile_lnk'], 'user.php?act=profile', 'info');
			...
			//把上传的临时文件移动到up目录下面 
	           ...
	           $headimgurl = 'http://'.$_SERVER['HTTP_HOST'].'/mobile/'.$adname;
		
	           $sql = 'UPDATE ' . $ecs->table('users') . " SET `headimg`='".$adname."'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";

	           $db->query($sql);//ec原生头像
		
		       $sql = 'UPDATE ' . $ecs->table('weixin_user') . " SET `headimgurl`='".$headimgurl."'  WHERE `ecuid`='" . $_SESSION['user_id'] . "'";
